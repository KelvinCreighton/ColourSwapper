...
def swap_colour_with_blur(current_colour, from_colour, to_colour, buffer):
    return ( to_colour[0] + buffer-(from_colour[0]-current_colour[0]), to_colour[1] + buffer-(from_colour[1]-current_colour[1]), to_colour[2] + buffer-(from_colour[2]-current_colour[2]) )
...
def colour_swap(img_path, colour1, colour2, tolerance, blur_swap_tolerance):
    ...
    for item in data:
        if colour_in_buffer_range(item[:3], colour1[:3], tolerance):    # Swap colour 1 with colour 2
            ...
            new_colour = swap_colour_with_blur(item[:3], colour1[:3], colour2[:3], blur_swap_tolerance)
            new_data.append(new_colour + item[3:])
        elif colour_in_buffer_range(item[:3], colour2[:3], blur_swap_tolerance):
            new_colour = swap_colour_with_blur(item[:3], colour2[:3], colour1[:3], blur_swap_tolerance)
            new_data.append(new_colour + item[3:])
        else:
